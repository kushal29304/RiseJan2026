...
SELECT * FROM Products WHERE SalesUnit > (Select SalesUnit FROM Products WHERE ProductID=5)
...
SELECT ProductName FROM Products WHERE SalesUnit > ALL(SELECT SUM(SalesUnit) FROM Products WHERE City = 'Surat')
...
SELECT * FROM Products WHERE Quantity > (SELECT AVG(Quantity) FROM Products p2 WHERE p2.City = p1.City)

--17. Find cities where the total salesUnit is higher than the total salesUnit of 'Vadodara'.
SELECT City FROM Products GROUP BY City HAVING SUM(salesUnit) > (SELECT SUM(salesUnit) FROM Products WHERE City = 'Vadodara')

--18. List products that are sold in the city that has the maximum variety (count) of products.
SELECT * FROM Products WHERE City = (SELECT TOP 1 City FROM Products GROUP BY City ORDER BY COUNT(*) DESC)
